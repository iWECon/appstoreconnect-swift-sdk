...
let package = Package(
    name: "AppStoreConnectSwiftSDK",
    platforms: [
        .iOS(.v13),
        .macOS(.v10_15)
    ],
    products: [
        .library(name: "AppStoreConnectSwiftSDK", targets: ["AppStoreConnectSwiftSDK"])
    ],
    dependencies: [
        .package(url: "https://github.com/CreateAPI/URLQueryEncoder.git", from: "0.2.0")
    ],
    targets: [
        .testTarget(name: "AppStoreConnect-Swift-SDK-Tests", dependencies: ["AppStoreConnectSwiftSDK"], path: "Tests"),
        .target(
            name: "AppStoreConnectSwiftSDK",
